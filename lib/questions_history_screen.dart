  ...
  void initState() {
    ...
    /*getDataBase().answerHolderDao.fetchAnswerHolderWithChildren(widget.fieldSet.form!).then((AnswerHolder? value) {
      if (value == null) {
        ...
        _answerHolder = AnswerHolder.fill(widget.fieldSet.form, false, false, widget.fieldSet.getName(context.locale.languageCode), null, DateTime.now().toString(), Uuid().v1());
        ...
        print("we already have an answer holder ${widget.fieldSet.id}");
        _newAnswer = false;
        _answerHolder = value;
        BlocProvider.of<QuestionsBloc>(context).add(QuestionsEvent.loadQuestions(widget.fieldSet.id!));
      }
