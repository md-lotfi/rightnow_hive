...
class _MultiSelectViewState extends State<MultiSelectView> with AutomaticKeepAliveClientMixin {
          ...
          children: [
            ...
            if (!widget.viewOnly)
              ...
              FormField(
                          ...
                          return Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                  ...
                                  width: 0.80,
                                  color: (widget.answerHolder?.answers?.length ?? 0) == 0 ? Colors.grey.shade300 : (_currentChoices == null ? Colors.red : Colors.grey.shade300)),
                            ),
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      for (Choice item in _currentChoices ?? [])
                                        Container(
                                          child: Text(item.getName(context.locale.languageCode)),
                                        ),
                                    ],
                                  ),
                                ),
                ...
                validator: (value) {
                  if (isRequired(widget.question)) if (_currentChoices == null) return FORM_LIST_NOT_SELECTED;
                  return null;
                },
              ),
            //),
  ...
  Widget _showSelect(FormFieldState<List<Choice>> state, Widget Function() builder) {
    return SearchableDropdown<Choice>(
      ...
      searchBuilder: (items, search) {
        return items.where((element) => element.getName(context.locale.languageCode).toLowerCase().contains(search.toLowerCase())).toList();
      },
      onSelected: (selectedValue) {
        print("on selected result 1 $selectedValue");
        List<MultiSelectAnswer> m = [];
        for (var item in selectedValue) {
          m.add(MultiSelectAnswer.fill(widget.answerHolder?.id, item.id, item.label));
        }
