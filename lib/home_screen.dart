...
import 'package:rightnow/components/scrollable_text.dart';
...
class HomePage extends StatelessWidget {
  ...
  _refreshLocalData(BuildContext context, ValueNotifier<int> dataChanged) async {
    ...
    await LocalDBUpdate(
      ...
      dataChanged.notifyListeners();
    });
  }

  Widget _body(BuildContext context) {
    ValueNotifier<int> _dataChanged = ValueNotifier(0);
    return RefreshIndicator(
      child: Column(children: [
                  ...
                  Positioned(
                    ...
                    child: Row(
                      children: [
                        IconButton(
                          onPressed: () async {
                            _refreshLocalData(context, _dataChanged);
                          },
                          icon: Icon(
                            Icons.refresh,
                            size: 30,
        ...
        ValueListenableBuilder(
          valueListenable: _dataChanged,
          builder: (context, value, child) => _content(context),
        ),
      ]),
      onRefresh: () async {
        _refreshLocalData(context, _dataChanged);
                    ...
                    if (snapshot.connectionState == ConnectionState.done) {
                      ...
                      for (SuperCategory item in snapshot.data ?? []) {
                        w.add(_topButton(item.title(context.locale.languageCode), item.icon, () async {
                          /*Map<String, dynamic> cats = await getDataBase<CategoryDao>().fetchCategoriesOfSuperCategory(context, item.id ?? -1);
                          if (cats.containsKey('categories')) {
                            if ((cats['categories'] as List).length == 1) {}
                          }*/
                          Navigator.push(
                            context,
                            MaterialPageRoute(
